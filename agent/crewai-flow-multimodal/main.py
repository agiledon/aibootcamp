    ...
    try:
        ...
        for text in texts:
            ...
            if not text or not text.strip():
                ...
                embedding = ollama_embed_model.get_text_embedding(text.strip())
    ...
    if _collection is None:
        ...
        if utility.has_collection(config.COLLECTION_NAME):
            ...
            try:
                ...
                _collection = Collection(config.COLLECTION_NAME, schema)
                _collection.create_index("embedding", {"index_type": "IVF_FLAT", "metric_type": "L2", "params": {"nlist": 1024}})
                _collection.load()
...
def check_system_status():
    ...
    try:
        ...
        if utility.has_collection(config.COLLECTION_NAME):
            collection = Collection(config.COLLECTION_NAME)
            collection.load()
            # Check if collection has data using num_entities
            try:
